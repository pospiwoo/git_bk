...
class SpliceGraph:
    ...
    def DynamicTable(self, curr_idx):
        ...
        for to_idx in curr_node.to_edges:
            ...
            if next_node.visited == 0:
                ...
                new_dist = curr_node.cov + next_node.cov
                if new_dist > self.DynamicP_score[curr_node.idx]:
                    self.DynamicP_track[next_node.idx] = curr_node.idx
                    self.DynamicP_score[next_node.idx] = new_dist
                    #print "curr_idx", curr_idx, to_idx
        curr_node.visited = 0
        return
        
    def trackDynamicPath(self, curr_idx):
        tmp_max_score = -1.0
        tmp_max_idx = -1
        for i in xrange(0,len(self.DynamicP_score)):
            if self.DynamicP_score[i] > tmp_max_score:
                tmp_max_score = self.DynamicP_score[i]
                tmp_max_idx = i
        tmp_prev_idx = tmp_max_idx
        while 1:
            curr_node = self.G[tmp_prev_idx]
            ...
            tmp_prev_idx = self.DynamicP_track[curr_node.idx]
            if tmp_prev_idx == -2 or tmp_prev_idx < 0:
                break
        self.max_seq_types = self.max_seq_types[::-1]
        self.max_seq = self.max_seq[::-1]
        self.max_seq_score = self.max_seq_score[::-1]
        self.max_seq_cov = self.max_seq_cov[::-1]
        return
    ...
    def grdQualityPath(self, curr_idx):
        curr_node = self.G[curr_idx]
        curr_node.visited = 1
        max_cov = -1.0
        next_idx = -1
