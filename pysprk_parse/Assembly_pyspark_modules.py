...
##        file_dataframe = sqlContext.createDataFrame(file_data, self.target_gene_list.append('FASTA'))
##        ["FASTA", "g_1", "g_2", "g_3"]
##        file_dataframe.show(5)
##        file_dataframe.printSchema()
#        
##        file_dataframe.select('FASTA').repartition(1).write.text(output_path)
##        file_dataframe.select('FASTA').coalesce(1).write.text(output_path)
##        file_dataframe.collect()
##        file_dataframe.select('FASTA').write.text(self.output_path)
##        file_dataframe.select('gene').write.text(self.output_path)
##        file_dataframe.select('coverage').write.csv(self.output_path)
#        file_dataframe.select('FASTA','gene','coverage').write.csv(self.output_path)
##        file_dataframe.write.csv(self.output_path)
##        file_dataframe.select('FASTA','gene','coverage').write.text(self.output_path)
##        file_dataframe.select('gene').write.csv(os.path.join(self.output_path,'coverage.csv'))
##        file_dataframe.select('gene','coverage').rdd.map(lambda x: ",".join(map(str, x))).repartition(1).saveAsTextFile(os.path.join(self.output_path,'coverage.csv'))
#        #.write.csv(os.path.join(self.output_path,'coverage.csv'))
##        query.repartition(1).write.csv("cc_out.csv", sep='|')
#        self.writeAccumulators(sc)

#        file_dataframe.select('gene','coverage').write.csv(os.path.join(self.output_path,'coverage'))
#        file_dataframe.select('gene','coverage').rdd.map(lambda row: [row[i] for i in [0,1]])\
#                             .saveAsTextFile(os.path.join(self.output_path,'coverage'))
        
#        file_dataframe.select('gene','coverage').rdd.flatMap(lambda row: [row[0], [x[0] for x in row[1]]])\
...
#                                            lambda s, d: for i in xrange(0,len(self.target_len_dic['PIK3CA_9'])): s[i]+d[i]), \
#                                            lambda s, d: for i in xrange(0,len(self.target_len_dic['PIK3CA_9'])): s[i]+d[i]),) \
#                             .filter(lambda x: x[0] == 'PIK3CA_9')\
#                            .aggregateByKey([0.0]*self.target_len_dic['PIK3CA_9'], \
#                                            (lambda s, x: map(add, s, x[1].value)), \
#                                            (lambda rdd1, rdd2: map(add, rdd1, rdd2[1].value))) \
...
#        .rdd.map(lambda x, y: map(add, x, y))
#        map(add, list1, list2)
#        .rdd.map(lambda x: ",".join(map(str, x))).repartition(1).saveAsTextFile(os.path.join(self.output_path,'coverage.csv'))       
#        file_dataframe.select('gene','coverage').groupBy("gene").filter(lambda x: x == '')

#        file_data.saveAsTextFile('/home/swoo/dev_pyspark/output_pyspark')
        #file_data.coalesce(1).saveAsTextFile('/home/swoo/dev_pyspark/output_pyspark')
        
#        with open('/home/swoo/AWS/output_pyspark_testing/testing_accm.txt','a') as accmOut:
    
#        for gene in self.target_cnt_dic:
#            if self.target_cnt_dic[gene].value == 0:
#                continue
#            print '111111111111111111 ', gene, self.target_cnt_dic[gene].value    
